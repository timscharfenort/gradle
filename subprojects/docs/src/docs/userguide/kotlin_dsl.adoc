...
val myOtherNewProperty by extra { "lazy initial value" }  // <2>
...
<2> Create a new extra property whose initial value is calculated when the property is accessed
...
That includes <<organizing_gradle_projects#sec:build_sources,buildSrc projects>>, <<composite_builds#composite_builds, included builds>> and link:{guidesUrl}/designing-gradle-plugins/[Gradle plugins].
